CREATE OR REPLACE FUNCTION public.get_exams_with_filters(filter_name_exam text DEFAULT NULL::text, filter_certification_id bigint DEFAULT NULL::bigint, filter_simulator boolean DEFAULT NULL::boolean, filter_active boolean DEFAULT NULL::boolean, filter_created_at date DEFAULT NULL::date, filter_created_at_op text DEFAULT '>='::text, order_by text DEFAULT 'created_at'::text, order_dir text DEFAULT 'DESC'::text, page integer DEFAULT 1, limit_value integer DEFAULT 10)
 RETURNS TABLE(id bigint, certification_id bigint, certification_name text, name_exam text, simulator boolean, time_limit integer, attempts integer, created_at timestamp with time zone, updated_at timestamp with time zone, active boolean, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
...
CREATE OR REPLACE FUNCTION public.get_exams_with_filters(
    ...
    filter_certification_name text DEFAULT NULL::text, -- 👈 NUEVO: Filtro por nombre de certificación
